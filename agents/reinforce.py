...
class Actor(nn.Module):
    ...
    def get_log_probs(self, x, actions):
        probs = self(x)
        return torch.log(probs.gather(-1, actions))
...
def train(args, decoder = None):
    ...
    actor = Actor(envs, args.hidden_size, args.state_dim, envs.envs[0].unwrapped.num_items, envs.envs[0].unwrapped.slate_size).to(args.device)
    ...
    for global_step in range(args.total_timesteps + 1):
        ...
        if start_ep and global_step % args.policy_frequency == 0:
            ...
            for _ in range(
                ...
                if args.observable:
                    observations_actor = data.observations
                else:
                    observations_actor = actor_state_encoder(data.observations)
                log_probs = actor.get_log_probs(observations_actor, data.actions.long())
                actor_loss = - torch.sum(returns_to_go * log_probs.sum(dim=-1))
